...
show_log_tail() {
  if [[ -f $DEBIAN_INSTALL_LOG_FILE ]]; then
    local log_lines=$((TERM_HEIGHT - LOGO_HEIGHT - 35))
    local max_line_width=$((LOGO_WIDTH - 4))

    tail -n $log_lines "$DEBIAN_INSTALL_LOG_FILE" | while IFS= read -r line; do
...
catch_errors() {
  ...
  gum style --foreground 1 --padding "1 0 1 $PADDING_LEFT" "Debian installation stopped!"
  show_log_tail
  ...
  gum style "$QR_CODE"
  echo
  gum style "Get help from yourself dumbass"
  ...
  while true; do
    ...
    if [[ -n ${DEBIAN_ONLINE_INSTALL:-} ]]; then
    ...
    case "$choice" in
    "Retry installation")
      bash ~/.local/share/debian/install.sh
      break
      ;;
    "View full log")
      if command -v less &>/dev/null; then
        less "$DEBIAN_INSTALL_LOG_FILE"
      else
        tail "$DEBIAN_INSTALL_LOG_FILE"
      fi
      ;;
