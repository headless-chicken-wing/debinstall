...
HISTSIZE=
HISTFILESIZE=
HISTIGNORE="ls:ll:la:lt:lz:cd:exit:clear:history"
...
#Source user's main functions files located inside ~/.local/lib/functions
if [ -d "$HOME/.local/lib/functions" ]; then
  for func in "$HOME/.local/lib/functions"/*.func; do
    if [ -f "$func" ]; then
      source "$func" || true
    fi
  done
...
if
 [[ -x /usr/bin/vivid ]]; then
  LS_COLORS="$(vivid generate molokai)"; elif
   [[ -f "$HOME/.local/lib/env/ls_colors_molokai" ]]; then
    . "$HOME/.local/lib/env/ls_colors_molokai"; elif
...
export PATH EDITOR BAT_THEME LS_COLORS PAGER TZ HISTIGNORE
